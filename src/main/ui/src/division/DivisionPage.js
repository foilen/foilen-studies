...
function DivisionPage() {
    ...
    function refreshScores() {
        autoRetry('Get Scores', () => window.service.divisionScores(), 5).then(response => {
    ...
    function updateFormValueAndCalculateAmount(e) {
        ...
        const divisorMaxInt = parseInt(newForm.divisorMax)
        const quotientMaxInt = parseInt(newForm.quotientMax)
        let smallest = divisorMaxInt < quotientMaxInt ? divisorMaxInt : quotientMaxInt
        let biggest = divisorMaxInt > quotientMaxInt ? divisorMaxInt : quotientMaxInt
        if (newForm.divisorAlwaysSmaller) {
            let all = newForm.divisorMax * newForm.quotientMax
    ...
    function startGame() {
        ...
        failuresToToast('Get Questions', () => window.service.divisionRandom(randomForm), false).then(response => {
            if (response.data.success) {
    ...
    function onAnswer() {
        ...
        // Send tracking
        autoRetry('Send tracking', () => window.service.divisionTrack({
            divisor: question[0],
            quotient: question[1],
            success: correct,
        }), 5)
        ...
        <div className="row">
            <div className="col col-sm-6 col-md-4 col-lg-4">
                    ...
                    <div className="row">
                        <div className="col-1">
                            <input type="checkbox" className="form-check-input"
                                   name="divisorAlwaysSmaller"
                                   checked={randomForm.divisorAlwaysSmaller}
                                   onChange={e => updateFormValueAndCalculateAmount(e)}
                            />
                        </div>
                        <div className="col-11">
                            <label htmlFor="divisorAlwaysSmaller">Diviseur toujours plus petit</label>
                        </div>
                    </div>
                ...
                {questions && <>
                    ...
                    <div className="row">
                        <div className="col-3">
                            <p>{questions[nextQuestion][0] * questions[nextQuestion][1]} ÷ {questions[nextQuestion][0]} =</p>
                        </div>
                        <div className="col-9">
                            <input type="number" className="form-control"
                                   autoComplete="off"
